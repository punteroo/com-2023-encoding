...
document
  ...
  .addEventListener("change", function (e) {
    ...
    switch (e.target.value) {
      ...
      case "MANCH":
        return (element.innerHTML = `Uno muy polenta, utilizado en las redes de hoy con el cable <b>Ethernet</b> (gracias Pato por el dato). Este método de codificación es el que <b>menos continuidad tiene</b>, y con mucha razón.<br/>Similar a como funciona el <b>RZ</b>, acá también el pulso durará la mitad del bit, pero introduciendo el uso de patrones y alternancia. En este caso, Manchester le asigna a todos los <b>1s</b> y <b>0s</b> un patrón de pulsos representativo, entonces podemos por ejemplo tener que el 1 es representado por medio pulso hacia arriba y medio hacia abajo, y el 0 es invertido a ese patrón.<img src="public/manchester-encoding-example.jpg"/>`);
      case "MANCHD":
        return (element.innerHTML = `Este también está muy bueno. Es muy, pero muy parecido a Manchester, con la diferencia que ahora no tienen patrones específicos cada bit. Existe un patrón global que se le asigna al comienzo de la comunicación a un bit, y luego existe un bit que alterna ese patrón (generalmente es el 1).<br/>Entonces, se comienza con un patrón, pero cada vez que se lee un bit <b>1</b>, se invierte y se alterna el patrón. Esto permite que la señal tenga mucha alternancia, y que no tenga problemas de continuidad.<img src="public/differential-manchester-encoding-example.jpg"/>`);
      case "AMI":
